...
import (
	...
	"github.com/jivelocity/femGo/internal/utils"
)
...
func (wh *WorkoutHandler) HandleUpdateWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	currentUser := middleware.GetUser(r)
	if currentUser == nil || currentUser == store.AnonymousUser {
		utils.WriteJSON(w, http.StatusBadRequest, utils.Envelope{"error": "you must be logged in to updated"})
		return
	}
	...
	if workoutOwner != currentUser.ID {
		utils.WriteJSON(w, http.StatusForbidden, utils.Envelope{"error": "your not authorized to update this workout"})
		return
	}
...
func (wh *WorkoutHandler) HandleDeleteWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		...
		utils.WriteJSON(w, http.StatusNotFound, utils.Envelope{"error": "invalid workout delete id"})
		return
	}
